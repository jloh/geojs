...
local function geoip_lookup(ip)
	...
	if ip_data['country']['iso_code'] then
		ip_data['country']["iso_code3"] = codes.country_code_3
				[ip_geo['country']['iso_code']]
	end                                 -- luacheck: ignore 631
...
function _M.geo_lookup(ip)
	...
	local res = {
		...
		["organization"]      = 'AS' ..
				table.concat({ lookup["autonomous_system_number"], lookup["autonomous_system_organization"] }, ' '), -- luacheck: ignore 631
